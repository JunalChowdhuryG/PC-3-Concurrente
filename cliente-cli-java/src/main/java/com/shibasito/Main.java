...
public class Main extends Application {
    ...
    // Usamos tu IP de Wi-Fi. Docker reenvía el puerto 5672 a RabbitMQ.
    ...
    private void ejecutarRpc(String routingKey, String payload) {
        ...
        // Usar una Tarea (Task) para ejecutar el RPC en un hilo de fondo
        Task<String> rpcTask = new Task<>() {
            @Override
            protected String call() throws Exception {
                // Prevenir 'race condition' (darle tiempo al backend de arrancar)
                // (Solo es útil la primera vez que se ejecuta)
                Thread.sleep(1000);
        ...
        rpcTask.setOnFailed(e -> {
            ...
            ex.printStackTrace();
            // Mostrar el error de una forma amigable
    ...
    private void mostrarRespuesta(String responseJson) {
        javafx.application.Platform.runLater(() -> {
            try {
                ...
                // Determinar el tipo de alerta basado en la respuesta
                ...
                // Usar un TextArea en el popup para respuestas largas
